...
Today the pipeline is HN-centric end to end: `fetch_feeds` → `union_feeds` → `list[Story]` keyed on an **`int` `hn_id`**, the sole anchor for dedupe, gist reuse, image reuse, and pruning ([fetch.py:208](backend/hackergist/fetch.py#L208), [store.py:84-101](backend/hackergist/store.py#L84-L101), [pipeline.py:194](backend/hackergist/pipeline.py#L194)).
...
> 2. **Across runs:** the gist + image are **sticky, reused by `story_key`** ([store.merge](backend/hackergist/store.py#L60)); `_stories_needing_gist` returns only stories with no existing gist, so a story already gisted is never re-fetched. The render fallback is keyed the same way.
>
> Scrape (article fetch + text/image extraction) and summarize happen in the same per-`Story` pass ([_gist_static](backend/hackergist/pipeline.py#L127)), so "not scraped twice" and "not summarized twice" are the same guarantee.
...
- **D4 (decided). Drop `feeds` from the contract** (never displayed; superseded by `discussions[].source`). `FEED_NAMES`/`normalize_feeds` ([models.py:45,322](backend/hackergist/models.py#L45)) move into `HackerNewsSource` or are removed.
...
- [ ] Create `backend/hackergist/sources/` (`base.py`): define `Post` (dataclass) and `FeedSource` (ABC: `name` + `get_posts()`).
...
- [ ] Move HN logic from [fetch.py](backend/hackergist/fetch.py) into `HackerNewsSource.get_posts()`: keep `_fetch_id_list`/`_fetch_items`/`_get_json` (the topstories+beststories id-list → hydrate path, [fetch.py:49-160](backend/hackergist/fetch.py#L49-L160)).
- [ ] **DELETE `fetch.union_feeds` + `fetch._merge_into`** ([fetch.py:208-256](backend/hackergist/fetch.py#L208-L256)) — they're not "moved", they're **rewritten at the `Post` level**. Reimplement HN's top/best intra-source dedupe inside `HackerNewsSource`: dedupe by `hn_id` then `canonical_url`; **`points` takes the MAX** across the two feeds (not first-non-null — both lists carry a score); fill `url`/`published` gaps; there is no longer a `feeds` field to union and no `author`.
- [ ] Map each HN item → `Post`: `link = item["url"] or None`, `comments_url = f"https://news.ycombinator.com/item?id={id}"` ([fetch.py:190](backend/hackergist/fetch.py#L190)), `points = score`, `published = iso_from_epoch(item["time"])`, `self_text = _strip_html(item["text"])`, `source = "hn"`. Reuses `_strip_html`, `domain_of`, `iso_from_epoch`.
- [ ] Keep the per-list 0-count WARNING ([fetch.py:107-112](backend/hackergist/fetch.py#L107-L112)) for observability.
...
## Phase 3 — Models / contract ([models.py](backend/hackergist/models.py))

- [ ] Add `Discussion` dataclass: `source`, `comments_url`, `clout: float`, `points: int | None`.
- [ ] Reshape `Story` ([models.py:226-254](backend/hackergist/models.py#L226-L254)): drop `hn_id`/`points`/`comments_url`/`author`/`feeds`; add `id: str`, `clout: float`, `discussions: list[Discussion]`. Keep `title`/`url`/`domain`/`published`/`image`/`gist`. Rename `hn_text` → `self_text` ([models.py:254](backend/hackergist/models.py#L254)) (still `repr=False, compare=False`, not serialized).
- [ ] Update `to_dict`/`from_dict` ([models.py:259-290](backend/hackergist/models.py#L259-L290)); bump `SCHEMA_VERSION = 2` ([models.py:42](backend/hackergist/models.py#L42)); rewrite the contract docstring ([models.py:7-32](backend/hackergist/models.py#L7-L32)).
- [ ] **Rename `GistKind` value `hn_text` → `self_text`** ([models.py:48](backend/hackergist/models.py#L48)). Rationale: [StoryCard.svelte:68](frontend/src/lib/components/StoryCard.svelte#L68) renders `gist.kind.replace('_',' ')` as a badge, so a lobste.rs self-post would literally show **"hn text"**. Touchpoints to change together: [extract.py:34](backend/hackergist/extract.py#L34) (`ExtractKind`), `GistKind` here, [types.ts:14](frontend/src/lib/types.ts#L14), [summarize.py:62](backend/hackergist/summarize.py#L62) (`_KIND_LABEL`), and the StoryCard badge. (Aside: `_KIND_LABEL` has no `title_only` entry and `summarize` returns `None` rather than emitting `title_only` despite its docstring — pre-existing, leave or tidy.)
- [ ] Remove `FEED_NAMES`/`normalize_feeds` ([models.py:45,322](backend/hackergist/models.py#L45)) from the contract (move into `HackerNewsSource` if still needed). Keep `canonical_url` ([models.py:104](backend/hackergist/models.py#L104)) + `domain_of` ([models.py:152](backend/hackergist/models.py#L152)) unchanged — `canonical_url` is the cross-source merge key.
...
- [ ] [pipeline.run](backend/hackergist/pipeline.py#L43): replace `fetch_feeds`+`union_feeds` ([pipeline.py:49-50](backend/hackergist/pipeline.py#L49-L50)) with `get_posts()` over the injected sources → `merge_posts`. Update the summary ([pipeline.py:65-78](backend/hackergist/pipeline.py#L65-L78)): `frontpage`/`best` counts → per-source counts (`hn`/`lobsters`).
- [ ] Re-key on `story_key` (was `s.hn_id`, int → str): [store.merge](backend/hackergist/store.py#L60) `existing_gists`/`existing_images` + sticky-reuse ([store.py:84-101](backend/hackergist/store.py#L84-L101)) and the `new_gists`/`new_images` `dict[int,…]`→`dict[str,…]` signatures; [_stories_needing_gist](backend/hackergist/pipeline.py#L194) + [_count_pruned](backend/hackergist/pipeline.py#L200); [_gist_new_stories](backend/hackergist/pipeline.py#L91)/[_gist_static](backend/hackergist/pipeline.py#L127)/[_gist_rendered](backend/hackergist/pipeline.py#L157) result maps + `by_id` ([pipeline.py:165](backend/hackergist/pipeline.py#L165)) + the **unresolved filter** `s.hn_id not in gisted` ([pipeline.py:115](backend/hackergist/pipeline.py#L115)); [render.render_pages](backend/hackergist/render.py#L40) return type `dict[int,str]`→`dict[str,str]`, local `out` ([render.py:56](backend/hackergist/render.py#L56)) and `return story.hn_id` ([render.py:66](backend/hackergist/render.py#L66)). The `published`-backfill ([store.py:103](backend/hackergist/store.py#L103)) stays. **Note:** gist reuse keyed on canonical URL ⇒ a cross-posted article is gisted once.
- [ ] [extract.extract](backend/hackergist/extract.py#L57): self-post branch reads `story.hn_text` ([extract.py:64-66](backend/hackergist/extract.py#L64-L66)) → `story.self_text`.
- [ ] [summarize.SYSTEM_PROMPT](backend/hackergist/summarize.py#L36) + `_KIND_LABEL` ([summarize.py:62](backend/hackergist/summarize.py#L62)): soften "Hacker News reader"/"Hacker News self-post" to source-neutral. Prompt stays cached.

## Phase 5 — Config & DI

- [ ] [config.py](backend/hackergist/config.py): add `lobsters_rss_url` (`https://lobste.rs/rss.rss`), `lobsters_hottest_url` (`https://lobste.rs/hottest.json`), `lobsters_enabled` (`HACKERGIST_LOBSTERS_ENABLED`, default `true`), lobste.rs `User-Agent`. Mirror `from_env` ([config.py:142-165](backend/hackergist/config.py#L142-L165)). Move `id_list_sources` ([config.py:167-178](backend/hackergist/config.py#L167-L178)) into `HackerNewsSource`.
- [ ] [di.py](backend/hackergist/di.py): `injector` has **no bare `list[T]` multibinding**. Add a small wrapper — `@dataclass class SourceRegistry: sources: list[FeedSource]` — and a `@singleton @provider def provide_sources(self, config) -> SourceRegistry` that builds `[HackerNewsSource(config), *([LobstersSource(config)] if config.lobsters_enabled else [])]`. `pipeline.run` resolves `injector.get(SourceRegistry).sources` (today it calls `fetch_feeds(config)` directly and takes no sources arg — change that).
...
- [ ] [README.md](README.md): "aggregates Hacker News" → "Hacker News + lobste.rs". [.env.example](.env.example): add `HACKERGIST_LOBSTERS_*`.
- [ ] Keep the contract in lockstep: [models.py](backend/hackergist/models.py) `to_dict` ↔ [types.ts](frontend/src/lib/types.ts) ↔ the one sample [.data/data.json](.data/data.json).
...
- **Ship it.** lobste.rs gisting is in; `HACKERGIST_LOBSTERS_ENABLED` defaults **on**. Still send a descriptive `User-Agent` + contact URL and keep the hourly cadence — practical politeness that avoids rate-limiting/blocking, regardless of any content directives.
...
- **Atomic multi-file change.** `to_dict` ([models.py:259](backend/hackergist/models.py#L259)) + `types.ts` Story + `isStory` ([data.ts:22](frontend/src/lib/data.ts#L22)) + the sample [.data/data.json](.data/data.json) must land together, or `isStory` drops every story (logs a warning, shows an empty feed).
...
- **`hn_id` is referenced in every tier — one atomic swap.** Grep `hn_id` exhaustively: [fetch.py](backend/hackergist/fetch.py), [store.py:84-101](backend/hackergist/store.py#L84-L101), [pipeline.py:115,122-123,140,154,165,194,200](backend/hackergist/pipeline.py#L115), [render.py:56,66](backend/hackergist/render.py#L56), [hotness.ts:59](frontend/src/lib/hotness.ts#L59), [+page.svelte:136](frontend/src/routes/+page.svelte#L136), [types.ts:28](frontend/src/lib/types.ts#L28), [data.ts:26](frontend/src/lib/data.ts#L26), every fixture. A missed int/str key mismatch only fails when a story flows through that path (e.g. render fallback → `KeyError` or a redundant re-gist).
- **Timestamp parsing.** Routing lobste.rs's offset-aware `pubDate` through `iso_from_epoch` ([models.py:76](backend/hackergist/models.py#L76)) → null `published` → backfilled to `generated_at` ([store.py:103](backend/hackergist/store.py#L103)) → every lobste.rs post looks brand-new. Use `feedparser.published_parsed` (already UTC).
